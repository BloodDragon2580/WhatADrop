local L = LibStub("AceLocale-3.0"):NewLocale("WhatADrop", "koKR")
if not L then return end

L["WhatADrop: Loaded"] = "WhatADrop: 로드됨"
L["WhatADrop: Enabled"] = "WhatADrop: 활성화"
L["WhatADrop: Disabled"] = "WhatADrop: 비활성화"
L["WhatADrop does not have any function command."] = "WhatADrop은 슬래시 명령어를 제공하지 않습니다."

L["This shows the level of the item you'll find in this week's chest."] = "목요일에 상자에서 나올 아이템 레벨을 표시합니다."
L["Weekly Chest Reward"] = "주간 상자 보상"

L["Loot Item Level: "] = "전리품 아이템 레벨: "
L["Weekly Chest Item Level: "] = "주간 상자 아이템 레벨: "
L["Weekly Residuum Amount: "] = "Weekly Residuum Amount:"