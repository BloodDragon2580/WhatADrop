local L = LibStub("AceLocale-3.0"):NewLocale("WhatADrop", "itIT")
if not L then return end

L["WhatADrop: Loaded"] = "WhatADrop: Caricato"
L["WhatADrop: Enabled"] = "WhatADrop: Abilitato"
L["WhatADrop: Disabled"] = "WhatADrop: Disabilitato"
L["WhatADrop does not have any function command."] = "WhatADrop non ha alcun comando valido."

L["This shows the level of the item you'll find in this week's chest."] = "Mostra l'item level dell'oggetto che troverai nel forziere di questa settimana."
L["Weekly Chest Reward"] = "Premio Forziere Settimanale"

L["Loot Item Level: "] = "Item Level Bottino: "
L["Weekly Chest Item Level: "] = "Item Level Forziere Settimanale: "
L["Weekly Residuum Amount: "] = "Settimanalmente Residuo Titanico:"